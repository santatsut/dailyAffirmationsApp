...
export default function SecondOnboardingScreen() {
  ...
  const toggleSelection = (id: string) => {
    if (selectedOption === id) {
      ...
      setSelectedOption(OPTIONS.find(option => option.id === id)?.label || "");
    }
  };

  const handleNext = async () => {
    try {
      console.log(`Selected age range ID: ${selectedOption}`);
      if (selectedOption) {
          await AsyncStorage.setItem('userAgeRange', selectedOption);
          router.push('/thirdOnboardingScreen');
      }
    } catch (error) {
      console.error('Error saving age range to AsyncStorage:', error);
    }
  };
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.subtitle}>
                This help us tailor your experience!
            </Text>
            <FlatList
                data={OPTIONS}
